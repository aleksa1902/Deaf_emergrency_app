...
class RescureDangerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sendInfoButton.setOnClickListener {
            ...
            val data = mapOf(
                "Koja je priroda opasnosti?" to dangerNatureEditText.text.toString(),
                "Koliko je osoba ili životinja uključeno?" to numEditText.text.toString(),
                "Gde se taćno nalaze osobe koje treba spasiti?" to currLocationEditText.text.toString(),
                "Jesu li osobe zarobljene ili imaju povrede?" to dangerEditText.text.toString(),
                "Je li područje stabilno ili postoji rizik od urušavanja?" to additionalInfoEditText.text.toString()
            )

            val firefighterData = FirefighterData()
            firefighterData.userId = userId.toString()
            firefighterData.type = "rescure_danger"
                ...
                object : LocationListener {
                    override fun onLocationChanged(location: Location) {
                        if(net){
                            ...
                            val sharedPreferences = getSharedPreferences("MyPrefs", MODE_PRIVATE)
                            FirebaseHelper.saveDataFirefighters(data, sharedPreferences.getString("token", null).toString(), this@RescureDangerActivity)
                        }else{
